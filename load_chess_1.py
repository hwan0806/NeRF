...
for i in range(950, 995 + 1):
    if i % 5 == 0:
        depth = cv2.imread('./data/nerf_llff_data/chess/depth/' + 'frame-{:06d}.depth.png'.format(i), -1) #16bit
        min = (depth.min()) / 255 #255로 나눠보기
        max = (depth.max()) / 255
        if min == 0.0:
            ...
            for i in range(depth.shape[0]):
                if min_list[i] != 0:
                    min = (min_list[i]) / 255
                    break
        if max == 257.0: #큰 값으로 정렬해서, 65535의 값이 나오지 않은 값을 추출하기
            depth = depth.reshape(-1)
        ...
        min_max = np.array([min, max]).reshape(1, 2)
        depth_list.append(min_max)
depth_arr = np.array(depth_list).reshape(10, -1)

final_arr = np.concatenate([pose_hwf, depth_arr], axis = 1)

txt = np.savetxt('./data/nerf_llff_data/chess/poses_bounds_1.txt', final_arr)
x = np.loadtxt('./data/nerf_llff_data/chess/poses_bounds_1.txt')
