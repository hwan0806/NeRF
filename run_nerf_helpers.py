...
# Misc -> 기타
img2mse = lambda x, y : torch.mean((x - y) ** 2) #Mean squared error
mse2psnr = lambda x : -10. * torch.log(x) / torch.log(torch.Tensor([10.])) #psnr
to8b = lambda x : (255*np.clip(x,0,1)).astype(np.uint8) #255 -> [0, 1], 8 bit


# Positional encoding (section 5.1)
class Embedder:
    def __init__(self, **kwargs): #kwargs = keyword arguments -> 한 번에 인수들 불러오기
        self.kwargs = kwargs
        self.create_embedding_fn() #함수 미리 정의
        
    def create_embedding_fn(self):
        embed_fns = []
        d = self.kwargs['input_dims']
        out_dim = 0
        if self.kwargs['include_input']: #input을 포함 = True -> output dimension에 input dimension을 포함한다.
            embed_fns.append(lambda x : x)
            out_dim += d #out_dim = out_dim + input_dims
            
        max_freq = self.kwargs['max_freq_log2']
        N_freqs = self.kwargs['num_freqs']
        
        if self.kwargs['log_sampling']: #log sampling = True
...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False):
        ...
        #[3, 256] + [256, 256] + [256, 256] + [256, 256] + [256, 256] + [256 + 3, 256](skip connection) + [256, 256] + [256, 256] + [256, 256] -> density가 나오기 전까지

        ### Implementation according to the official code release (https://github.com/bmild/nerf/blob/master/run_nerf_helpers.py#L104-L105)
        self.views_linears = nn.ModuleList([nn.Linear(input_ch_views + W, W//2)]) #[3 + 256, 128] -> direction + feature space
    ...
    def load_weights_from_keras(self, weights): #weight 초기화
...
# Ray helpers
def get_rays(H, W, K, c2w): #ray 얻기, image height, image width, focal length(pixel 단위), c2w 
