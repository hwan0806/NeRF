...
#depth scaling -> 5000으로 나누어 m 단위로 변환
depth_array_list = []
for depth in depth_list:
    depth_image = cv2.imread('./data/nerf_llff_data/ICL_NUIM2/depth/' + depth, -1)
    min_max = np.array([5000 * focal_length / (depth_image.max() * 255), 5000 * focal_length / (depth_image.min() * 255)])
...
# txt = np.savetxt('./data/nerf_llff_data/ICL_NUIM2/poses_bounds_5.txt', npy_file)

# npy = np.loadtxt('./data/nerf_llff_data/ICL_NUIM2/poses_bounds_5.txt', txt)
