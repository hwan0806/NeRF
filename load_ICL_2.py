...
for depth in depth_list:
    ...
    min_max = np.array([depth_image.min()/50, depth_image.max()/50]) #cm
    depth_array_list.append(min_max)
depth_array = np.array(depth_array_list)

npy_file = np.concatenate([poses, depth_array], axis = 1)

txt = np.savetxt('./data/nerf_llff_data/ICL_NUIM2/poses_bounds_2.txt', npy_file)

npy = np.loadtxt('./data/nerf_llff_data/ICL_NUIM2/poses_bounds_2.txt', txt)
