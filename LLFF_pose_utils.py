...
def gen_poses(basedir, match_type, factors=None):
    ...
    files_needed = ['{}.bin'.format(f) for f in ['cameras', 'images', 'points3D']] #cameras.bin, images.bin, points3D.bin 파일이 모두 필요한 것 같다.
    if os.path.exists(os.path.join(basedir, 'sparse/0')): #만약에 basedir + 'sparse/0' path가 존재하면 True, 존재하지 않으면 False
        files_had = os.listdir(os.path.join(basedir, 'sparse/0'))
    else:
        files_had = []
    if not all([f in files_had for f in files_needed]): #files_needed > files_had
        print( 'Need to run COLMAP' )
        run_colmap(basedir, match_type)
    else: #files_had = files_needed -> COLMAP을 돌릴 필요 없다. 우리가 필요한 것은 cameras.bin, images.bin, points3D.bin 파일인 듯
        print('Don\'t need to run COLMAP')
        
    print( 'Post-colmap')
    
    poses, pts3d, perm = load_colmap_data(basedir) #colmap data를 load한다.
    
    save_poses(basedir, poses, pts3d, perm) #pose를 save한다.
