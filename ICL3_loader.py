...
rt = np.loadtxt('./data/nerf_llff_data/ICL_NUIM/Rt.txt')
...
#depth -> 16bit로 받아오기
depth_list = []
for depth in os.listdir('./data/nerf_llff_data/ICL_NUIM/depth'):
...
for depth in depth_list:
    depth_image = cv2.imread('./data/nerf_llff_data/ICL_NUIM/depth/' + depth, -1)
    min_max = np.array([depth_image.min()/255, depth_image.max()/255]) #255로 나눈다.
...
txt = np.savetxt('./data/nerf_llff_data/ICL_NUIM/poses_bounds_pose_change.txt', npy_file)

npy = np.loadtxt('./data/nerf_llff_data/ICL_NUIM/poses_bounds_pose_change.txt', txt)

np.save('./data/nerf_llff_data/ICL_NUIM/poses_bounds.npy', npy)